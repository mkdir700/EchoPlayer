...
/**
 * V2 架构更新通知状态类型
 * V2 Architecture Update Notification State Type
 */
...
/**
 * V2 架构更新通知管理Hook
 * V2 Architecture Update Notification Management Hook
 *
 * 管理应用版本更新通知的状态和逻辑。
 ...
 * - 自动初始化检查 / Automatic initialization check
 * - 智能版本比较 / Intelligent version comparison
 *
...
export function useUpdateNotification(): UpdateNotificationState {
  ...
  const compareVersions = useCallback((current: string, latest: string): boolean => {
    ...
    // 简单的版本比较逻辑，可以根据需要改进 / Simple version comparison logic, can be improved as needed
