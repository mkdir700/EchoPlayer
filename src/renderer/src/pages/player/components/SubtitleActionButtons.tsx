...
import { Edit3, Languages, Loader2, MessageCircle, MoreHorizontal } from 'lucide-react'
 ...
 * - 更多操作（打开右键菜单）
 */
function SubtitleActionButtons({
  subtitle,
  index,
  visible,
  isCurrentlyTranslating = false,
  onClick
}: SubtitleActionButtonsProps) {
  ...
  return (
    <Container $visible={visible}>
      ...
      <ActionButton
        ...
        title={isCurrentlyTranslating ? '正在翻译中...' : '翻译此字幕'}
        aria-label="翻译字幕"
        disabled={isCurrentlyTranslating}
        $loading={isCurrentlyTranslating}
      >
        {isCurrentlyTranslating ? <LoadingSpinner size={14} /> : <Languages size={14} />}
      </ActionButton>
...
const ActionButton = styled.button<{ $loading?: boolean }>`
  ...
  background: ${(p) =>
    p.$loading
      ...
      : 'var(--color-fill-2, rgba(255, 255, 255, 0.08))'};
  color: ${(p) => (p.$loading ? 'white' : 'var(--color-text-2, #999)')};
