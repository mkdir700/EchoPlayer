...
import { isMac, isWin } from '@renderer/infrastructure'
import { useShortcuts } from '@renderer/infrastructure/hooks/system/useShortcust'
import { initialState, useShortcutsStore } from '@renderer/state/stores/shortcuts.store'
...
const ShortcutSettings: FC = () => {
  ...
  const isValidShortcut = (keys: string[]): boolean => {
    ...
    const hasModifier = keys.some((key) =>
      ['CommandOrControl', 'Ctrl', 'Alt', 'Meta', 'Shift'].includes(key)
    )
    const hasNonModifier = keys.some(
      (key) => !['CommandOrControl', 'Ctrl', 'Alt', 'Meta', 'Shift'].includes(key)
    )
  ...
  const isDuplicateShortcut = (newShortcut: string[], currentKey: string): boolean => {
    return shortcuts.some(
      (s) =>
        s.key !== currentKey &&
        s.shortcut.length > 0 &&
        s.shortcut.join('+') === newShortcut.join('+')
    )
  }
  ...
  const handleKeyDown = (e: React.KeyboardEvent, record: Shortcut) => {
    ...
    updateShortcut({ ...record, shortcut: keys })
    setEditingKey(null)
  }
  ...
  const columns: ColumnsType<Shortcut> = [
    ...
    },
    {
      ...
      width: '50px',
      render: (record: Shortcut) => (
        <Switch size="small" checked={record.enabled} onChange={() => toggleShortcut(record.key)} />
      )
    }
  ]
